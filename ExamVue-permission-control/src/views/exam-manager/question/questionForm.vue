<template>
  <div class="app-container">
    ...
    <el-form ref="myForm"
      ...
      <el-form-item v-if="form.type == '判断题'"
                    label="题目答案"
                    prop="answer">
        <el-radio v-model="form.answer"
                  label="true">正确</el-radio>
        <el-radio v-model="form.answer"
                  label="false">错误</el-radio>
      </el-form-item>
    </el-form>
    <!-- 添加选项 -->
    ...
    <!-- 表格 -->
    <el-table v-if="form.type == '单选题' || form.type == '多选题'"
              ref="myTable"
              :data="form.options"
              border>
      <el-table-column prop="isRight"
                       label="是否答案"
                       width="100"
                       align="center">
        <template slot-scope="scope">
          <el-checkbox v-model="form.options[scope.$index].isRight"
                       :true-label="1"
                       :false-label="0">答案</el-checkbox>
        </template>
      </el-table-column>

      <el-table-column prop="abc"
                       label="编号"
                       width="55"
                       align="center" />
    ...
    <div slot="footer"
      ...
      <el-button icon="el-icon-back"
                 @click="$emit('close-event')">返 回</el-button>
...
import { saveQuestion } from '@/api/admin/question'
export default {
  name: 'QuestionForm',
  props: {
    curQuestion: {
    ...
    handleSave() {
      ...
      this.$refs.myForm.validate((valid) => {
        if (!valid) {
          ...
          // 将表单和选项数据一起上传
          saveQuestion({ question: this.form, option: this.form.options }).then(
            (res) => {
              if (res.status === 200) {
