<template>
  <div class="app-container">
    ...
    <div class="filter-container"
      ...
      <!-- 题库选择 -->
      <el-select v-model="queryParam.repoId"
                 filterable
                 placeholder="所属题库"
                 clearable
                 class="filter-item"
      ...
      <!-- 章节选择 -->
      <el-select v-model="queryParam.chapterId"
                 filterable
                 placeholder="所属章节"
                 clearable
                 class="filter-item"
      ...
      <el-button-group v-if="!son"
                       class="filter-item"
                       style="float:  right">
        <el-button icon="el-icon-upload2"
                   @click="importVisible = true">导入</el-button>
        <el-button icon="el-icon-download"
                   @click="myExport">导出</el-button>
      </el-button-group>
    </div>

    <!-- 抽屉框，题目编辑表单 -->
    <el-drawer :title="curQuestion == null ? '增加题目' : '修改题目'"
               :visible.sync="drawerVisable"
               size="60%"
               :wrapper-closable="false"
               :destroy-on-close="true">
      <questionForm v-if="drawerVisable"
                    :cur-question="curQuestion"
                    @close-event="drawerVisable = false; queryPage()" />
    </el-drawer>

    <!-- 导入框 -->
    <el-dialog title="导入试题"
               :visible.sync="importVisible"
               width="30%">
      <el-row>
        <el-button type="warning"
                   @click="downloadImport">下载导入模板</el-button>
    ...
    <el-table ref="multipleTable"
      ...
      <el-table-column prop="content"
        ...
        <template slot-scope="scope">
          ...
          <span v-else
                style="color: #409eff">{{ scope.row.content }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="chaperName"
                       label="所属章节"
                       align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    downloadImport() {
      window.open(
        'https://wangpeng-imgsubmit.oss-cn-hangzhou.aliyuncs.com/Exam/%E9%A2%98%E7%9B%AE%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF.xlsx'
      )
    },
