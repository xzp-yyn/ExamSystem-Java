<template>
  <div class="app-container">
    <!--弹出框-->
    <el-drawer title="添加题目"
               :visible.sync="drawerVisable"
               size="70%"
               :wrapper-closable="false"
               :destroy-on-close="true">
      <QuestionList v-if="drawerVisable"
                    :son="true"
                    :type-id="curTypeId"
    ...
    <el-dialog title="随机抽题"
      ...
      <el-form :model="form"
        ...
        <el-form-item label="选择题库">
          <el-select v-model="form.repoId"
                     ...
                     clearable
                     @change="repoChange">
            <el-option v-for="item in repoData"
                       :key="item.id"
                       :label="item.name"
                       :value="item.id" />
          </el-select>
        ...
        <el-form-item label="选择章节">
          <el-select v-model="form.chapterId"
                     placeholder="请选择章节"
                     clearable>
            <el-option v-for="item in chapterData"
                       :key="item.id"
                       :label="item.name"
                       :value="item.id" />
          </el-select>
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
                   @click="submitRandomForm">确 定</el-button>
      </div>
    </el-dialog>
    <el-row :gutter="10">
      <el-col ref="leftObject"
              :span="5"
              :style="styleObj">
        <el-card>
          <p>试卷名称：<el-input v-model="paperData.name"
                      style="width: 60%" /></p>
          ...
          <p>及格分数：<el-input v-model="paperData.qualifyScore"
                      type="number"
                      size="mini"
                      style="width: 70px" /> 分</p>
          <el-button type="danger"
                     size="mini"
                     style="width: 47%"
                     @click="$router.push({ name: 'Paper' })">返回</el-button>
          <el-button type="primary"
                     size="mini"
                     style="width: 47%"
          ...
          <div v-for="(item, i) in 5"
               :key="i">
            <el-divider><el-button type="text"
                         @click="jumpTypeCard(i)">{{ typeInfo[i].name }}</el-button></el-divider>
            <div>
              共有 <span class="number">{{ typeInfo[i].count }}</span> 题，
              共有 <span class="number">{{ typeInfo[i].score }}</span> 分，
              设置每题<el-input v-model="typeInfo[i].eachScore"
                        type="number"
                        size="mini"
                        style="width: 70px"
                        @change="handlerEach(i)" />分
            </div>
          </div>
        </el-card>
      </el-col>
      ...
      <el-col :span="18"
              :offset="6">
        <el-card v-for="(type, i) in questionList"
                 ref="typeCard"
                 :key="i"
                 :style="i !== 0 ? 'margin-top: 20px' : null">
          <div slot="header">
            <span>{{ typeInfo[i].name }}</span>
            <el-button style="float: right; padding: 3px 0; margin-left: 10px"
                       type="text"
                       @click="handlerRandomQuestion(i)">随机抽题</el-button>
            <el-button style="float: right; padding: 3px 0"
                       type="text"
                       @click="handlerAddQuestion(i)">添加题目</el-button>
          </div>
          <!-- 一道题 -->
          <div v-for="(question, j) in type"
               :key="question.id">
            <el-row>
              <el-col :span="20">
                <div>
                  <el-tag effect="dark"
                          size="medium">{{ j + 1 }}</el-tag>
                  <span class="question-content">{{ question.content }}</span>
                </div>
                <el-image v-if="question.image != null"
                          fit="contain"
                          :src="question.image"
                ...
                <div v-if="i == 3 || i == 4">
                  <div class="grayback-box">
                    ...
                    <el-image v-if="question.answerImage != null"
                              fit="contain"
                ...
                <div class="grayback-box">
                  ...
                  <el-image v-if="question.analysisImage != null"
                            fit="contain"
              ...
              <el-col :span="4"
                ...
                <div style="margin-top: 20px">
                  ...
                  <el-button type="primary"
                             ...
                             icon="el-icon-bottom"
                             circle
                             @click="handleDown(i, j)" />
                  <el-button type="danger"
                             size="small"
                             icon="el-icon-delete"
                             circle
                             @click="handleRemove(i, j)" />
                </div>
                <div style="margin-top: 20px">
                  本题 <el-input v-model="question.score"
                            type="number"
                            size="mini"
                            style="width: 70px" /> 分
                </div>
...
import { savePaper, getPaper } from '@/api/admin/paper'

import { getRandomQuestion } from '@/api/admin/question'
  ...
  methods: {
    ...
    handlerSave() {
      ...
      savePaper({
        ...
        paperQuestionListList: this.questionList,
      }).then((res) => {
        if (res.status === 200) {
