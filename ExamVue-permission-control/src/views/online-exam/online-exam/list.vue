  ...
  <div class="app-container">
    ...
    <el-dialog title="考试信息"
      ...
      <p v-if="curExam.privilege === 1">
        输入密码：<el-input v-model="curPassword"
                  placeholder="请输入密码"
                  style="width: 300px" />
      </p>
      <div slot="footer"
        ...
        <el-button type="primary"
                   @click="submitStart">确 定</el-button>
      </div>
    </el-dialog>
      ...
      <el-table-column fixed="right"
        ...
        <template slot-scope="scope">
          <el-button type="success"
                     size="small"
                     @click="handleStart(scope.row)">开始考试</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      if (this.curExam.privilege === 1) {
        checkPassword({
          ...
          if (res.status !== 200) {
            ...
            // 创建学生考试请求
            createStudentExam({ studentId: that.id, examId: that.curExam.id })
              .then((res) => {
                if (res.status === 200) {
                  this.$message({
