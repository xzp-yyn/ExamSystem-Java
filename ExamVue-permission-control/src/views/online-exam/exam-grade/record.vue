<template>
  <div class="app-container">
    ...
    <div class="filter-container"
      ...
      <!-- 考试列表 -->
      <el-select v-model="queryParam.examId"
                 placeholder="请选择考试">
        <el-option v-for="item in examData"
                   :key="item.id"
                   :label="item.name"
                   :value="item.id" />
      </el-select>
      <!-- 是否完成 -->
      <el-select v-model="queryParam.isFinished"
                 placeholder="是否完成">
        <el-option label="考试中"
                   :value="0" />
        <el-option label="考试完成"
                   :value="1" />
      </el-select>
      <!-- 是否批改 -->
      <el-select v-model="queryParam.isChecked"
                 placeholder="是否批改">
        <el-option label="等待批改主观题"
                   :value="0" />
        <el-option label="批改完成"
                   :value="1" />
      </el-select>
      <!-- 是否及格 -->
      <el-select v-model="queryParam.isQualify"
                 placeholder="是否及格">
        <el-option label="不及格"
                   :value="0" />
        <el-option label="及格"
    ...
    <el-table ref="multipleTable"
      ...
      <el-table-column prop="isFinished"
                       label="是否完成"
                       align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.isFinished === 0"
                style="color: red">考试中</span>
          <span v-else
                style="color: green">考试完成</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="isChecked"
                       ...
                       align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.isChecked === 0"
                style="color: orange">等待批改主观题</span>
          <span v-else
                style="color: green">批改完成</span>
        </template>
      </el-table-column>

      <el-table-column prop="qualifyScore"
                       label="及格分数"
                       align="center" />
      ...
      <el-table-column prop="isQualify"
                       ...
                       align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.isQualify === 0"
                style="color: red">不及格</span>
          <span v-else
                style="color: green">及格</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button v-permission="['admin']"
                     ...
                     @click="handleCheck(scope.row)">批改试卷</el-button>
          <el-button type="success"
                     size="small"
                     :disabled="!scope.row.isFinished"
                     @click="handleDetail(scope.row)">查看试卷答题情况</el-button>
          <el-button v-permission="['admin']"
                     type="danger"
                     size="small"
...
import { queryExams } from '@/api/admin/exam'
...
import { queryRecordByPageByStudent } from '@/api/student/record'
...
export default {
  ...
  methods: {
    ...
    handleCheck(item) {
      ...
      this.$router.push({
        name: 'ExamDetail',
        params: { studentExamId: item.id, myState: 1 },
      })
    },
    ...
    handleDetail(item) {
      ...
      this.$router.push({
        name: 'ExamDetail',
        params: { studentExamId: item.id, myState: 0 },
      })
    },
